...
class JMeterThread @JvmOverloads constructor(
    ...
    private suspend fun executeSamplePackage(
        ...
        if (result != null && !result.isIgnore) {
            ...
            result.groupThreads = nbActiveThreadsInThreadGroup
            result.allThreads = nbTotalActiveThreads
            result.threadName = threadName
            val subResults = result.subResults
            if (subResults != null) {
                for (subResult in subResults) {
                    subResult.groupThreads = nbActiveThreadsInThreadGroup
                    subResult.allThreads = nbTotalActiveThreads
                    subResult.threadName = threadName
                }
            }
    ...
    private fun doEndTransactionSampler(
        ...
        val transactionResult = transactionSampler.transactionResult.also {
            it.threadName = threadName
            it.groupThreads = threadGroup.numberOfThreads
            it.allThreads = JMeterContextService.getNumberOfThreads()
        }
        ...
        for (assertion in assertions) {
            ...
            if (assertion is AbstractScopedAssertion) {
                ...
                if (assertion.isScopeChildren(scope) || assertion.isScopeAll(scope)) {
                    val children = parent.subResults
                    ...
                    if (childError && parent.isSuccessful) {
                        val assertionResult =
                            AssertionResult((assertion as AbstractTestElement).name)
                        assertionResult.setResultForFailure("One or more sub-samples failed")
                        parent.addAssertionResult(assertionResult)
                        parent.isSuccessful = false
                    }
                }
            } else {
                processAssertion(parent, assertion)
